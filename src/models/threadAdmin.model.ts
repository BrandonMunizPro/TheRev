...
export class ThreadAdminModel {
  ...
  async grantAdmin(
    data: GrantThreadAdminInput,
    userId: string
  ): Promise<ThreadAdmin> {
    if (!data.threadId || !data.suggestedUserId) {
      throw ErrorHandler.missingRequiredFields(['threadId', 'suggestedUserId']);
    }
    ...
    const suggestedUser = await this.permissionsService.checkUserExists(
      data.suggestedUserId
    );

    const grantedThreadAdminPrivilege =
      await this.dao.grantOrRestoreThreadAdmin(
        suggestedUser.id,
        data.threadId,
        userId
      );
  ...
  async revokeAdmin(
    ...
    if (!data.authorId) {
      throw ErrorHandler.missingRequiredField('authorId');
    }

    return await this.dao.revokeThreadAdmin(data.authorId, data.threadId);
  }

  async listAdminsForThread(
    data: ThreadQueryInput,
    userId: string
  ): Promise<ThreadAdmin[]> {
    if (!data.id) {
      throw ErrorHandler.missingRequiredField('threadId');
