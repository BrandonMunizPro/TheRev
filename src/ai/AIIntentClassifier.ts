...
const DEFAULT_INTENT_PATTERNS: IntentPattern[] = [
  ...
  },
  {
    ...
    patterns: [
      ...
      /youtube\s+(.+)/i,
      /^(find|watch|see|look)\s+(for\s+)?(.+)/i,
      /^(what\s+is|who\s+is|when\s+did|where\s+is)\s+/i,
      /^(how\s+to|how\s+do|how\s+can)\s+/i,
      /^(show\s+me|find\s+me|get\s+me)\s+/i,
      /^(play|watching|viewing)\s+(.+)/i,
      /^(latest|new|recent)\s+(.+)/i,
      /^(top\s+|best\s+)(.+)/i,
      /(.+)\s+(highlights|compilation|mixes?|edits?)/i,
    ],
    keywords: [
      'go to',
      'navigate to',
